...
class Model:
    def __init__(self):
        ...
        self.model = DualStreamModel(self.seq_len, feature_dim, embed_dim, num_heads, hidden_dim_transformer, dropout_transformer, num_transformer_layers, latent_dim_transformer, hidden_dim_cnn, latent_dim_cnn)
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.kl_mean = None
        self.kl_std = None
    
    def load(self):
        self.model.load_state_dict(torch.load(pathlib.Path(__file__).parent / "final_model_weights.pt"))
        self.model.to(self.device)
        self.model.eval()

        kl_score_mean_std = torch.load(pathlib.Path(__file__).parent / "kl_score_mean_std.pt")
        self.kl_mean = kl_score_mean_std["kl_mean"]
        self.kl_std = kl_score_mean_std["kl_std"]
    ...
    def batch_predict(self, batch):
        with torch.no_grad():
            _, _, _, _, _, kl_loss, _, _ = self.model(batch)
            return kl_loss
    ...
    def predict(self, X):
        ...
        for i in range(num_batches):
            max_idx = min((i + 1) * self.batch_size, X.shape[0])
            batch = X[i * self.batch_size:max_idx]
            kl_scores.append(self.batch_predict(batch))
        kl_scores = torch.cat(kl_scores, dim=0)
        kl_scores = (kl_scores - self.kl_mean) / self.kl_std
        return torch.sigmoid(kl_scores).flatten().cpu().numpy()
